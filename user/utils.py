...
from django_shop.settings import EMAIL_HOST_USER
...
def send_code(email, key='email_verification'):
    ...
    text = f"Hello," \
           ...
           f"Thank you, Mahdi Malvandi shopping website"

    send_mail(title, text, EMAIL_HOST_USER, [email])
    cache_key = f'{key}:{email}'

    cache.set(cache_key, verification_code, 60 * 3)
    return {'email': email, 'message': 'verification code sent successfully'}


def check_verification_code(data: dict, key='email_verification'):
